    ...
    val hourlyTips = fares
      ...
      .reduce(
        (f1: (Long, Float), f2: (Long, Float)) => {
          (f1._1, f1._2 + f2._2) // 预聚合 ? ?
        },
        new WrapWithWindowInfo())

    // This is straightforward, but has the drawback that it’s buffering all of the TaxiFare objects in the windows
    // until the windows are triggered, which is less efficient than computing the sum of the tips incrementally,
    // using a reduce or agggregate function.


    //    val hourlyTips2 = fares
    //      .map((f: TaxiFare) => (f.driverId, f.tip))
    //      .keyBy(_._1)
    //      .timeWindow(Time.hours(1))
    //      .reduce(
